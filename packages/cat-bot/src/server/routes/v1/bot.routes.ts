...
botRouter.get('/:id/commands', (req, res) => { void botSessionConfigController.getCommands(req, res); });

// PUT /api/v1/bots/:id/commands/:name — toggles a single command on/off.
botRouter.put('/:id/commands/:name', (req, res) => { void botSessionConfigController.toggleCommand(req, res); });

// GET /api/v1/bots/:id/events — lists all registered event modules with their enabled status.
botRouter.get('/:id/events', (req, res) => { void botSessionConfigController.getEvents(req, res); });

// PUT /api/v1/bots/:id/events/:name — toggles a single event module on/off.
botRouter.put('/:id/events/:name', (req, res) => { void botSessionConfigController.toggleEvent(req, res); });
