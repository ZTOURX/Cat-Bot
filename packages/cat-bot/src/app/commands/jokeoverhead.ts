/**
 * /jokeoverhead — Joke Over Head Avatar Overlay
 *
...
export const config: CommandConfig = {
  ...
  role: Role.ANYONE,
  author: 'AjiroDesu',
  description: "The joke went over a user's head.",
  category: 'fun',
  usage: '[@user]',
  cooldown: 5,
  hasPrefix: true,
...
export const onCommand = async ({ chat, user, event }: AppCtx): Promise<void> => {
  ...
  try {
    ...
    await chat.replyMessage({
      ...
      attachment: [{ name: 'jokeoverhead.png', stream: Buffer.from(await res.arrayBuffer()) }],
    });
  } catch (err) {
