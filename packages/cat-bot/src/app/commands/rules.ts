...
async function getRulesList(db: AppCtx['db'], threadID: string): Promise<string[]> {
  const coll = db.threads.collection(threadID);
  if (!(await coll.isCollectionExist('rules'))) await coll.createCollection('rules');
...
export const button = {
  ...
  [BUTTON_ID.confirmRemove]: {
    ...
    onClick: async ({
      chat, event, session, button: btn, db,
    }: AppCtx): Promise<void> => {
...
export const onCommand = async ({
  chat, event, args, db, thread, state, native, button: btn, prefix, usage,
}: AppCtx): Promise<void> => {
  ...
  if (!type) {
    ...
    const msgId = await chat.replyMessage({ style: MessageStyle.MARKDOWN, message: body });
  ...
  if (['add', '-a'].includes(type)) {
    ...
    if (!args[1]) {
      await chat.replyMessage({ message: '⚠️ Please enter the content for the rule you want to add.' });
      return;
    }
  ...
  if (['edit', '-e'].includes(type)) {
    ...
    if (isNaN(stt)) {
      await chat.replyMessage({ message: '⚠️ Please enter the number of the rule you want to edit.' });
      return;
    }
    if (!rulesList[stt - 1]) {
    ...
    if (!args[2]) {
      await chat.replyMessage({ message: `⚠️ Please enter the new content for rule #${stt}.` });
      return;
    }
  ...
  if (['move', '-m'].includes(type)) {
    ...
    if (isNaN(n1) || isNaN(n2)) {
      await chat.replyMessage({ message: '⚠️ Please enter the numbers of the 2 rules to swap.' });
      return;
    }
    if (n1 === n2) {
      await chat.replyMessage({ message: '⚠️ Cannot swap a rule with itself.' });
      return;
    }
    ...
    rulesList[n1 - 1] = tmp;
    await saveRulesList(db, threadID, rulesList);
    await chat.replyMessage({ message: `✅ Swapped rule #${n1} and rule #${n2} successfully.` });
    return;
  }
  ...
  if (['delete', 'del', '-d'].includes(type)) {
    ...
    if (isNaN(n)) {
      await chat.replyMessage({ message: '⚠️ Please enter the number of the rule to delete.' });
      return;
    }
    if (!rulesList[n - 1]) {
      await chat.replyMessage({
  ...
  if (['remove', 'reset', '-r', '-rm'].includes(type)) {
    if (!(await isThreadAdmin(thread, senderID))) {
      await chat.replyMessage({ message: '❌ Only admins can remove all group rules.' });
      return;
    }
    ...
    if (isButtonPlatform(native.platform)) {
      const confirmId = btn.generateID({ id: BUTTON_ID.confirmRemove, public: false });
      const cancelId  = btn.generateID({ id: BUTTON_ID.cancelRemove,  public: false });
      ...
      await chat.replyMessage({
        style:   MessageStyle.MARKDOWN,
        message: '⚠️ Are you sure you want to remove **all** group rules? This cannot be undone.',
  ...
  if (!isNaN(Number(type))) {
    ...
    for (const sttStr of args) {
      ...
      if (!isNaN(n) && rulesList[n - 1]) lines.push(`${n}. ${rulesList[n - 1]}`);
    }
    if (!lines.length) {
      await chat.replyMessage({
        style:   MessageStyle.MARKDOWN,
...
export const onReply = {
  [STATE.view_rule]: async ({
    chat, session, event, state: stateApi, db,
  }: AppCtx): Promise<void> => {
    stateApi.delete(session.id);

    const { botMsgId, prefix } = session.context as { botMsgId: string; prefix: string };
    const threadID = event['threadID'] as string;
    const body     = (event['message'] as string | undefined) ?? '';

    const num = parseInt(body);
    if (isNaN(num) || num < 1) {
      await chat.replyMessage({ message: '⚠️ Please enter a valid rule number.' });
      return;
    }
...
const handleConfirmRemove = async ({
  chat, session, state: stateApi, db, event,
}: AppCtx): Promise<void> => {
  stateApi.delete(session.id);
  const threadID = event['threadID'] as string;
  await saveRulesList(db, threadID, []);
  await chat.replyMessage({ message: '✅ All group rules have been removed successfully.' });
};

export const onReact: Record<string, (ctx: AppCtx) => Promise<void>> = Object.fromEntries(
