...
export async function upsertSessionCommands(userId: string, platform: string, sessionId: string, commandNames: string[]): Promise<void> {
  ...
  const toCreate = commandNames.filter((name) => !existingNames.has(name)).map((commandName) => ({
    userId, platformId, sessionId, commandName, isEnable: true,
  }));
  if (toCreate.length > 0) await prisma.botSessionCommand.createMany({ data: toCreate });
}
...
export async function setCommandEnabled(userId: string, platform: string, sessionId: string, commandName: string, isEnable: boolean): Promise<void> {
  ...
  await prisma.botSessionCommand.upsert({
    where: { userId_platformId_sessionId_commandName: { userId, platformId, sessionId, commandName } },
